...
- [x] `engage` (`Activity`/`Article`) → Destinations & Local Lifestyle: add translations, per-locale slugs/meta, media galleries, OSM coordinates, workflow status — migration `000008_activity_translations`: consolidated `activities` (card) + `articles` (detail) into one entity; parent gains destination location fields (lat/lng/address/opening_info JSONB, nullable for Local Lifestyle) + `type` index; `activity_translations` table (title/slug/brief/body/meta + workflow status, UNIQUE(locale,slug)); backfilled en-US from existing data (body pulled from linked articles); repo rewritten to JOIN + published-only + optional `type` filter + pagination; service uses `i18ncontent.NormalizeLocale`; handler reads `?locale=`/`Accept-Language` + `?type=`. `articles` table now unreferenced (cleanup drop deferred). Media galleries + admin C/U/D deferred
