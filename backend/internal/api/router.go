...
import (
	...
	// Swagger 2.0 docs (generated by swaggo/swag; served at /admin/swagger/*).
	// The blank import registers the generated docs package so fiber-swagger
	// can read the embedded spec. Gate the UI route below behind JWT auth.
	_ "jingdezhen-ceramics-backend/internal/docs"
...
func SetupRoutes(
	...
	/* --- API docs: Swagger UI (PRD §2.4.2) ---
	// Served at /admin/swagger/* behind JWT auth (any authenticated staff).
	// The spec is generated by `swag init` (Makefile target `swag`) from
	// swaggo/swag annotations on the handlers + the general API info in
	// cmd/api/main.go. Pilot: public catalog (/catalog/*) only; the rest of
	...
	/* --- Public analytics event endpoint (PRD §3.4.2, TDD §263) ---
	// Consent-gated (cookie_analytics, checked by IP hash in the service). Rate
	// limited 60/min/IP per TDD §333 (Fiber in-process limiter — fine for the
	// single-VPS MVP). Registered BEFORE the empty-prefix checkoutGroup: Fiber
	// v2 attaches `Group("").Use()` at the radix root and leaks it to every
	// route registered after, so a public route must precede that group. */
	analyticsGroup := app.Group("/analytics", limiter.New(limiter.Config{
		Max:          60,
