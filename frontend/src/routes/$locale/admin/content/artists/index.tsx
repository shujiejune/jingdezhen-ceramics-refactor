...
export const Route = createFileRoute('/$locale/admin/content/artists/')({
...
function ArtistsListPage() {
  ...
  const [list, setList] = useState<Artist[] | null>(null)
  ...
  const columns: Column<Artist>[] = [
    {
      ...
      cell: (row) => (
        <DetailLinkCell to={`/${locale}/admin/content/artists/${row.id}`} label={row.name} />
      ),
    },
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-[1.1rem] font-semibold text-ink-900">{t('admin.content.artists')}</h2>
      </div>
