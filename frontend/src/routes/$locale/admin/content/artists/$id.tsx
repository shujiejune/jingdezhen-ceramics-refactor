...
import type { Artist, ContentStatus } from '~/lib/types'

export const Route = createFileRoute('/$locale/admin/content/artists/$id')({
...
function ArtistDetailPage() {
  ...
  const { push } = useToast()
  const [item, setItem] = useState<Artist | null | undefined>(undefined)
  ...
  const save = async () => {
    ...
    setErr(null)
    try {
      const updated = await api.adminUpdateArtist(token, Number(id), {
        name,
        slug,
        bio,
  ...
  const workflow = async (action: 'submit' | 'approve' | 'reject' | 'unpublish') => {
    ...
    try {
      const fn = {
        submit: api.adminSubmitArtist,
        approve: api.adminApproveArtist,
        reject: api.adminRejectArtist,
        unpublish: api.adminUnpublishArtist,
      }[action]
  ...
  if (item === null) {
    return (
      <div className="py-32 text-center text-[0.88rem] text-ink-400">
        ...
        <Link to={`/${locale}/admin/content/artists` as never} className="mt-4 link-quiet">
          {t('admin.common.back')}
        </Link>
      </div>
    )
  ...
  return (
    <div>
      <Link
        to={`/${locale}/admin/content/artists` as never}
        className="inline-flex items-center gap-1.5 text-[0.84rem] text-ink-500 hover:text-cobalt-700"
      >
        <ArrowLeft size={14} /> {t('admin.content.artists')}
      </Link>

      <div className="mt-4 flex items-center gap-3">
        <h2 className="text-[1.1rem] font-semibold text-ink-900">{item.name}</h2>
