    ...
    <div className="mx-auto max-w-shell px-4 pt-8 sm:px-6">
      <Breadcrumbs
        items={[
          ...
          ...(product.artist_slug
            ? [
                {
                  label: product.artist_name ?? '',
                  to: `/${locale}/artists/$slug`,
                  params: { slug: product.artist_slug },
                },
              ]
            : []),
          { label: product.title },
      ...
      <div className="grid gap-12 lg:grid-cols-[1fr_1fr] xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="lg:pt-2">
          ...
          {skus.length > 0 && (
            <div className="mt-7">
              <h2 className="label-base">
                {skus.length > 1 ? t('product.selectVariant') : t('product.buyingOptions')}
              </h2>
              <div className="flex flex-col gap-2">
                {skus.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      <span>
                        <span className="block text-[0.9rem] font-medium text-ink-800">
                          {label || s.sku_code}
                        </span>
              ...
              <CertificateChip
                onClick={() =>
                  void navigate({
                    ...
                    params: { locale, code: product.cert_code! },
                  })
                }
              />
            </div>
          ...
          <div className="mt-8 rounded-xl border border-cobalt-100 bg-wash/60 p-5">
            ...
            <dl className="mt-3 grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
              {attrRows.map(([k, v]) => (
                <div
                  key={k}
                  className="flex items-baseline justify-between gap-3 border-b border-cobalt-100/60 pb-1.5"
                >
      ...
      {product.description && (
        <section className="mx-auto mt-16 max-w-2xl">
          ...
          <h2 className="text-center text-[1.3rem] font-semibold text-ink-800">
