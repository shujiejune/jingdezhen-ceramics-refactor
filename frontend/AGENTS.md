...
- **Styling:** Tailwind CSS v3.4 + PostCSS + autoprefixer. **NOT v4** (v4 is a
  CSS-first config rewrite; deferred). Design tokens from TDD §6 (celadon/ink
  palette, spacing, type scale) as CSS variables in `src/styles/tokens.css`,
  surfaced as Tailwind theme extensions.
...
- **HTTP:** a thin API client in `src/lib/api.ts` (to be built) over `redaxios`
  or `fetch`. Base URL from env; Bearer-token interceptor; typed error surface.
...
src/
  ...
  routes/
    __root.tsx        root layout: providers (QueryClient, i18n, auth) + <Outlet>
    [locale]/          public routes, locale-segment (en-US | zh-CN)
      __root.tsx      locale layout: i18n provider, hreflang/canonical, nav
      index.tsx       landing
    ...
    auth/             login, signup, activate, forgot/reset-password (+ 2FA verify)
